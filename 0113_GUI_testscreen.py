...
class LoadingBar(RelativeLayout):
    ...
    def _update_graphics(self, *args):
        ...
        # Center the percentage label inside the bar - absolute positioning
        self.percent_label.pos = (0, bar_y)  # 从父控件左边缘开始
        self.percent_label.size = (self.width, bar_height)  # 占满整个宽度
...
class MotorControlScreen(MDScreen):
    ...
    def build_ui(self):
        ...
        # Project name card (top center)
        project_card = MDCard(
            style="elevated",
            size_hint=(None, None),
            size=(dp(400), dp(60)),  # 卡片尺寸
            radius=[dp(16)],
            elevation=4,
            pos_hint={"center_x": 0.5, "center_y": 0.9}
        )
        ...
        layout.add_widget(project_card)
        ...
        # Abort Test button (below progress bar) - RED COLOR
        # abort_button = MDButton(
        #     MDButtonText(
        #         text="Abort Test",
        #         font_style="Title",
        #         theme_text_color="Custom",  # 允许自定义颜色
        #         text_color=(1, 1, 1, 1)     # 白色文字
        #     ),
        #     md_bg_color=(0.8, 0.1, 0.1, 1),  # 红色背景
        #     pos_hint={"center_x": 0.5, "center_y": 0.28},
        #     size_hint=(0.3, None),
        #     height=dp(56),
        #     on_press=self.on_stop_clicked
        ...
        abort_button = MDButton(
            ...
            style="filled",
            theme_bg_color="Custom",  # 🔑 关键：启用自定义背景色
            md_bg_color=(0.8, 0.1, 0.1, 1),  # Red background
            pos_hint={"center_x": 0.5, "center_y": 0.28},
            size_hint=(0.3, None),
            height=dp(56),
            #on_press=self.on_stop_clicked
            on_press=self.show_stop_confirm_dialog
        )
        layout.add_widget(abort_button)
        ...
        # ========== Time Card (next to date card) ==========

        self.time_card = MDCard(
            size_hint=(None, None),
            size=(dp(100), dp(40)),
            #pos_hint={"x": 0.2, "y": 0.02},  # 日期卡片右边
        ...
        layout.add_widget(self.time_card)
        ...
        logo = FitImage(
            ...
            size_hint=(None, None), size=(dp(233), dp(103)),
    ...
    def show_stop_confirm_dialog(self, instance):
        """Show confirmation dialog when user clicks Stop button"""
        self.stop_dialog = MDDialog(
            MDDialogIcon(icon="alert-circle-outline"),
            MDDialogHeadlineText(text="Are you sure you want to stop the experiment?"),
            MDDialogButtonContainer(
                MDButton(
                ...
                MDButton(
                    ...
                    style="text",
                    on_release=self.confirm_stop_action
                ),
                spacing="8dp",
            ),
        )
    ...
    def confirm_stop_action(self, instance):
        """确认停止后的实际操作"""
        # 关闭对话框
        self.stop_dialog.dismiss()
        
        # 执行停止操作
        print("⏹️  Abort Test confirmed")
        self.stop_requested = True
        
        # 停止LoadingBar的倒计时
        if hasattr(self, 'loading_bar') and hasattr(self.loading_bar, '_event'):
            self.loading_bar._event.cancel()
